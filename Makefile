...
cargo-build-release:
	...
	git add --force target/aarch64-apple-darwin/release/web-service-chat-axum
	cargo build --release --target x86_64-pc-windows-gnu
	git add --force target/x86_64-pc-windows-gnu/release/web-service-chat-axum.exe
	TARGET_CC=x86_64-unknown-linux-gnu cargo build --release --target x86_64-unknown-linux-gnu
	git add --force target/x86_64-unknown-linux-gnu/release/web-service-chat-axum
	TARGET_CC=x86_64-unknown-linux-musl cargo build --release --target x86_64-unknown-linux-musl
	git add --force target/x86_64-unknown-linux-musl/release/web-service-chat-axum
...
podman-build:
	...
	podman build -t web-service-chat-axum "${top}"
